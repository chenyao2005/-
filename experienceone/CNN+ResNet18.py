...
# 设置增强类型: "none" / "basic" / "strong"
aug_type = "basic"
# 1. 全局参数
data_path = "./data/PlantDoc"
train_path = os.path.join(data_path, "TRAIN")
test_path = os.path.join(data_path, "TEST")
result_path = f"./results/resnet18_transfer_{aug_type}"
os.makedirs(result_path, exist_ok=True)


num_classes = 27
batch_size = 32
img_size = 224   # ResNet18 期望 224x224
epochs_stage1 = 10  # 冻结阶段
epochs_stage2 = 30  # 微调阶段
use_weighted_sampler = True  # 类别严重不均衡时，设为 True
...
def run_one_stage(model, train_loader, val_loader, epochs, optimizer, criterion, stage_name="Stage"):
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(result_path, f"best_resnet18_{stage_name.lower()}.pth"))
...
if __name__ == "__main__":
    print("开始 ResNet18 学习率消融实验：")
    classes = test_dataset.classes

    # 定义要测试的学习率组合
    stage1_lrs = [1e-3, 5e-4, 2e-3]   # 阶段1学习率
    stage2_lrs = [1e-4, 5e-5, 2e-4]   # 阶段2学习率

    for lr1 in stage1_lrs:
        for lr2 in stage2_lrs:
            print(f"实验: Stage1 lr={lr1}, Stage2 lr={lr2}")
            ...
            optimizer_stage1 = optim.Adam(model.fc.parameters(), lr=lr1)
            run_one_stage(model, train_loader, val_loader, epochs_stage1,
                          optimizer_stage1, criterion, stage_name="Stage1_FC")

            # 加载阶段 1 的最佳权重
            model.load_state_dict(torch.load(
                os.path.join(result_path, "best_resnet18_stage1_fc.pth")))

            # 阶段 2：解冻全网络，小学习率微调
            for param in model.parameters():
                param.requires_grad = True

            optimizer_stage2 = optim.Adam(model.parameters(), lr=lr2, weight_decay=1e-5)
            run_one_stage(model, train_loader, val_loader, epochs_stage2,
                          optimizer_stage2, criterion, stage_name="Stage2_Finetune")

            # 评估：加载阶段 2 的最佳权重（如果阶段 2 未保存，用阶段 1 权重）
            best_path = os.path.join(result_path, "best_resnet18_stage2_finetune.pth")
            if not os.path.exists(best_path):
                best_path = os.path.join(result_path, "best_resnet18_stage1_fc.pth")
            model.load_state_dict(torch.load(best_path))

            print("在测试集上评估：")
            evaluate_model(model, test_loader, classes, prefix="测试集")
            print("结束，结果已保存到", result_path)
"""
# 5. 主流程：两阶段训练（Batch Size 消融实验）
if __name__ == "__main__":
    print("开始 ResNet18 批量大小消融实验：")
    classes = test_dataset.classes
    ...
    for bs in batch_sizes:
        ...
        # 用当前 batch size 重新构建 DataLoader
        if use_weighted_sampler:
            sampler = build_weighted_sampler(train_dataset)
...
if __name__ == "__main__":
    ...
    # 加载阶段 1 的最佳权重
    model.load_state_dict(torch.load(os.path.join(result_path, "best_resnet18_stage1_fc.pth")))
